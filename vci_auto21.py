...
class VCIGeneratorGUI:
    ...
    def setup_discharge_tab(self):
        # 상단 고정 프레임 생성 (높이 고정)
        top_frame = ttk.Frame(self.discharge_tab, height=60)
        top_frame.grid(row=0, column=0, sticky='ew', padx=10, pady=5)
        top_frame.grid_propagate(False)  # 크기 고정
        ...
        paste_button.place(relx=0.5, rely=0.3, anchor='center')
        
        # 빈 영역 클릭 시 붙여넣기를 위한 레이블 추가 (절대 위치)
        empty_area = ttk.Label(top_frame, text="Click here to paste data")
        empty_area.place(relx=0.5, rely=0.7, anchor='center')
        
        # 빈 영역 클릭 이벤트 바인딩
        empty_area.bind('<Button-1>', lambda e: self.handle_paste('discharge'))
        
        # 컨테이너 라인 프레임 (상단 프레임 아래에 위치)
        self.discharge_lines_frame = ttk.Frame(self.discharge_tab)
        self.discharge_lines_frame.grid(row=1, column=0, sticky='ew', padx=10, pady=5)
    ...
    def setup_load_tab(self):
        # 상단 고정 프레임 생성 (높이 고정)
        top_frame = ttk.Frame(self.load_tab, height=60)
        top_frame.grid(row=0, column=0, sticky='ew', padx=10, pady=5)
        top_frame.grid_propagate(False)  # 크기 고정
        ...
        self.load_lines_frame.grid(row=1, column=0, sticky='ew', padx=10, pady=5)
        ...
        # General Remark 추가
            ...
            for name, entry in self.terminal_efficiency_entries.items():
                ...
                if name in ['Planner_Time_Msc', 'Planner_Time_LP', 'Terminal_Time'] and value:
                    try:
                        # convert_datetime 함수를 사용하여 날짜/시간 형식 변환
