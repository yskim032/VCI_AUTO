...
class VCIGeneratorGUI:
    ...
    def setup_operations_tab(self):
        ...
        gang_split_frame.pack(pady=5, fill='x', padx=70)
        ...
        gang_work_frame.pack(pady=5, fill='x', padx=70, anchor='w')
    ...
    def generate_xml(self):
        try:
            ...
            for line in self.gang_split_lines:
                ...
                # crane.set('projected', '100')  # 기본값
                crane.set('final', line['work_time'].get())
                # crane.set('stevedoresuperintendent', 'John Smith')  # 기본값
                # crane.set('gangforeman', 'Mike Johnson')  # 기본값
                # crane.set('terminal', 'KRPUSPN')  # 기본값
        ...
        # General Remark 추가
            if self.general_remark_text.get("1.0", "end-1c").strip():
                generalremarks = SubElement(summary, 'generalremarks')
                
                # 각 줄을 별도의 generalremark 요소로 처리
                for line in self.general_remark_text.get("1.0", "end-1c").strip().split('\n'):
                    if line.strip():  # 빈 줄 제외
                        generalremark = SubElement(generalremarks, 'generalremark')
                        generalremark.set('comment', line.strip())

            # # Husbandry
            # husbandry = SubElement(summary, 'husbandry')
    ...
    def setup_terminal_efficiency_tab(self):
        ...
        for item_id, label_text in efficiency_items:
            ...
            if item_id in ['Planner_Time_Msc', 'Planner_Time_LP', 'Terminal_Time']:
    ...
    def handle_terminal_efficiency_paste(self):
        ...
        try:
            # 이미지의 데이터를 직접 입력
            default_data = {
                'Terminal_Time': '20250412 1947',
                'CI_Proforma': '5',
                'CI_Planner': '4.6',
                'CI_Gang_Availability': '4.53',
                'CI_Sub_Optimal': 'Nil',
                'Starting_Operations': '1',
                'Quantity_Of_Containers': '0',
                'Changes_After_Ingate': '0',
                'Moved_from_Another_Terminal': '116',
                'Qty_of_Containers_before_Arrival': '2',
                'Live_Connections': '0',
                'Reason_for_Live_Connections': '2',
                'Affect_by_Cut_And_Run': '0',
                'Reason_For_Cut_and_Run': '1',
                'Average_Yard_Utilisation': '70',
                'Remarks': 'Nil'
            }

            # 기본 데이터 입력
            for field_name, value in default_data.items():
                if field_name in self.terminal_efficiency_entries:
                    entry = self.terminal_efficiency_entries[field_name]
                    entry.delete(0, tk.END)
                    entry.insert(0, value)

            # 클립보드에서 추가 데이터가 있다면 처리
            try:
                clipboard_data = self.root.clipboard_get()
                if clipboard_data.strip():
                    ...
                    for line in lines:
                        ...
                        if len(parts) >= 2:
                            ...
                            if item_name in self.terminal_efficiency_entries:
                                self.terminal_efficiency_entries[item_name].delete(0, tk.END)
                                self.terminal_efficiency_entries[item_name].insert(0, item_value)
            except:
                pass  # 클립보드 데이터가 없거나 처리할 수 없는 경우 무시
