        ...
        if len(content) >= 8:
            ...
            if entry == self.vesundoc_entry and len(content) >= 13:  # YYYYMMDD HHMM 형식
                try:
                    # 날짜와 시간 파싱
                    date_str = content[:8]
                    time_str = content[9:13]
                    dt = datetime.strptime(f"{date_str} {time_str}", "%Y%m%d %H%M")
                    
                    # Unmooring From 시간 계산 (-5분)
                    from_dt = dt - timedelta(minutes=5)
                    from_time = from_dt.strftime("%H%M")
                    self.dep_mooring_from_entry.delete(0, 'end')
                    self.dep_mooring_from_entry.insert(0, f"{date_str} {from_time}")
                    
                    # Unmooring To 시간 계산 (+10분)
                    to_dt = dt + timedelta(minutes=10)
                    to_time = to_dt.strftime("%H%M")
                    self.dep_mooring_to_entry.delete(0, 'end')
                    self.dep_mooring_to_entry.insert(0, f"{date_str} {to_time}")
