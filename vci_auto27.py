...
class VCIGeneratorGUI:
    ...
    def save_to_json(self):
        try:
            ...
            # Arrival tab data
            data['arrival'] = {
                'docatter': self.docatter_entry.get(),
                'tugs': []
            }
            ...
            # Departure tab data
            data['departure'] = {
                'docatter': self.docatter_entry_departure.get(),
                'tugs': []
            }
            ...
            for frame in self.container_frames_discharge:
                container_data = {
                    'container_no': frame.container_no_entry.get(),
                    'size': frame.size_entry.get(),
                    'type': frame.type_entry.get(),
                    'weight': frame.weight_entry.get(),
                    'discharge_time': frame.discharge_time_entry.get()
                }
            ...
            for frame in self.container_frames_load:
                container_data = {
                    ...
                    'type': frame.type_entry.get(),
                    'weight': frame.weight_entry.get(),
                    'load_time': frame.load_time_entry.get()
                }
            ...
            for frame in self.shifting_frames:
                move_data = {
                    'container_no': frame.container_no_entry.get(),
                    'from': frame.from_entry.get(),
                    'to': frame.to_entry.get(),
                    'time': frame.time_entry.get()
                }
                data['shifting']['lid_moves'].append(move_data)
            
            # Terminal Efficiency tab data
            data['terminal_efficiency'] = {
                'starting_operations': self.starting_operations_entry.get(),
                'total_containers': self.total_containers_entry.get(),
                'total_moves': self.total_moves_entry.get(),
                'total_time': self.total_time_entry.get(),
                'total_containers_per_hour': self.total_containers_per_hour_entry.get(),
                'total_moves_per_hour': self.total_moves_per_hour_entry.get()
            }
            
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=4)
                
            messagebox.showinfo("Success", "데이터가 성공적으로 저장되었습니다.")
    ...
    def load_from_json(self):
        try:
            ...
            if 'header' in data:
                ...
                self.general_remark_entry.delete(0, tk.END)
                self.general_remark_entry.insert(0, header.get('general_remark', ''))
