...
class VCIGeneratorGUI:
    def __init__(self, root):
        ...
        self.file_menu.add_command(label="Save", command=self.save_to_json)
        self.file_menu.add_command(label="Load", command=self.load_from_json)
        
        # Help menu
        self.help_menu = tk.Menu(self.menubar, tearoff=0)
        self.menubar.add_cascade(label="도움말", menu=self.help_menu)
        self.help_menu.add_command(label="Version 정보", command=self.show_version_info)
        ...
        # 생성 버튼들을 상단에 배치
        self.generate_button = ttk.Button(button_frame, text="Generate XML", command=self.generate_xml)
        self.generate_button.pack(side='left', padx=5)
        
        self.generate_partner_button = ttk.Button(button_frame, text="Generate XML (Partner)", command=self.generate_xml_partner)
        self.generate_partner_button.pack(side='left', padx=5)
        ...
        # 붙여넣기 횟수와 컨테이너 수 추적을 위한 변수 추가
        self.discharge_paste_count = 0
        self.load_paste_count = 0
        self.discharge_operator_counts = {}  # {operator: [paste1_count, paste2_count, ...]}
        self.load_operator_counts = {}  # {operator: [paste1_count, paste2_count, ...]}
    ...
    def create_datetime_entry(self, parent, label_text, entry_name=None):
        ...
        if label_text == "Sailed From Berth":
    ...
    def handle_date_change(self, entry):
        ...
        if len(content) >= 8:
            ...
            # Sailed from berth 시간이 변경되었을 때 Unmooring 시간 자동 계산
            if entry == self.vesundoc_entry and len(content) >= 13:  # YYYYMMDD HHMM 형식
                try:

                    # Sailed From Berth 시간 파싱
                    sailed_time = datetime.strptime(content, "%Y%m%d %H%M")

                    # Pilot 시간 계산 (Sailed From Berth - 30분)
                    pilot_from = sailed_time - timedelta(minutes=30)
                    pilot_to = sailed_time + timedelta(hours=1)
                    ...
                    # Tug 시간 계산 (Sailed From Berth - 1시간 ~ + 1시간)
                    tug_from = sailed_time - timedelta(hours=1)
                    tug_to = sailed_time + timedelta(hours=1)
                    ...
                    #Unmooring From 시간 계산 (-5분)
                    unmooring_from = sailed_time - timedelta(minutes=5)
                    unmooring_to = sailed_time + timedelta(minutes=10)

                    if hasattr(self, 'dep_mooring_from_entry') and self.dep_mooring_from_entry is not None:
                        self.dep_mooring_from_entry.delete(0, tk.END)
                        self.dep_mooring_from_entry.insert(0, unmooring_from.strftime("%Y%m%d %H%M"))

                    if hasattr(self, 'dep_mooring_to_entry') and self.dep_mooring_to_entry is not None:
                        self.dep_mooring_to_entry.delete(0, tk.END)
                        self.dep_mooring_to_entry.insert(0, unmooring_to.strftime("%Y%m%d %H%M"))   

                    
                except ValueError:
                    pass  # 잘못된 날짜/시간 형식인 경우 무시
    ...
    def setup_arrival_tab(self):
        ...
        # Draft + Mooring 섹션
        # Create frame to hold both draft frames side by side
        draft_container = ttk.Frame(self.arrival_tab)
        draft_container.pack(pady=5, fill='x', padx=5)
        
        # Left draft frame
        draft_frame = ttk.LabelFrame(draft_container, text="Draft", width=200)
        draft_frame.pack(pady=5, padx=5,side='left', fill='both', expand=True)
        
        # Right draft frame (copy)
        draft_frame_copy = ttk.LabelFrame(draft_container, text="Mooring", width=200) 
        draft_frame_copy.pack(pady=5, padx=5, side='right', fill='both', expand=True)
        ...
        mooring_fields = [("From", "arr_mooring_from_entry"), ("To", "arr_mooring_to_entry")]
        for label_text, entry_name in mooring_fields:
            self.create_datetime_entry(draft_frame_copy, label_text, entry_name)
        ...
        def update_all_from_docatter(*args):
            try:
                ...
                # Mooring 시간 설정
                mooring_from = docatter - timedelta(minutes=5)  
                mooring_to = docatter + timedelta(minutes=10)  
                
                # Mooring 엔트리 업데이트
                if hasattr(self, 'arr_mooring_from_entry'):
                    self.arr_mooring_from_entry.delete(0, tk.END)
                    self.arr_mooring_from_entry.insert(0, mooring_from.strftime("%Y%m%d %H%M"))
                
                if hasattr(self, 'arr_mooring_to_entry'):
                    self.arr_mooring_to_entry.delete(0, tk.END)
                    self.arr_mooring_to_entry.insert(0, mooring_to.strftime("%Y%m%d %H%M"))
    ...
    def add_tug_frame(self, parent_frame, tab_type):
        ...
        # 첫 번째 Tug의 시간 값을 복사
        if tab_type == 'arrival' and self.arr_tug_entries:
            first_tug = self.arr_tug_entries[0]
            from_entry.insert(0, first_tug["From"].get())
            to_entry.insert(0, first_tug["To"].get())
            comment_entry.insert(0, first_tug["Comment"].get())
        elif tab_type == 'departure' and self.dep_tug_entries:
            first_tug = self.dep_tug_entries[0]
            from_entry.insert(0, first_tug["From"].get())
            to_entry.insert(0, first_tug["To"].get())
            comment_entry.insert(0, first_tug["Comment"].get())
    ...
    def setup_operations_tab(self):
        ...
        frame.pack(fill='x', pady=3)  # pady 추가
    ...
    def setup_departure_tab(self):
        ...
        frame = ttk.Frame(timeline_frame)
        frame.pack(pady=5, fill='x')
        ttk.Label(frame, text="Departure Bowthruster", width=35).pack(side='left')
        ...
        self.dep_reaforanc_var = tk.StringVar(value="Authorities")
        ...
        #Draft + Mooring 섹션
        draft_container_departure = ttk.Frame(self.departure_tab)
        draft_container_departure.pack(pady=5, fill='x', padx=5)

        # Draft 섹션
        draft_frame_departure = ttk.LabelFrame(draft_container_departure, text="Draft")
        draft_frame_departure.pack(pady=5, padx=5,side='left', fill='both', expand=True)

        # Mooring 섹션
        mooring_frame_departure = ttk.LabelFrame(draft_container_departure, text="Unmooring")
        mooring_frame_departure.pack(pady=5, padx=5,side='right', fill='both', expand=True)

        mooring_fields = [("From", "dep_mooring_from_entry"), ("To", "dep_mooring_to_entry")]
        for label_text, entry_name in mooring_fields:
            self.create_datetime_entry(mooring_frame_departure, label_text, entry_name)


        draft_fields = [("AFT", "dep_draft_aft_entry"), ("FWD", "dep_draft_fwd_entry")]
        for label_text, entry_name in draft_fields:
            frame = ttk.Frame(draft_frame_departure)
            frame.pack(pady=5, fill='x')
            ttk.Label(frame, text=label_text, width=15).pack(side='left')
            entry = ttk.Entry(frame, width=6)  # 흘수값 (예: 11.45)
            entry.pack(side='left', padx=145)
            setattr(self, entry_name, entry)
    ...
    def setup_discharge_tab(self):
        ...
        # 요약 정보를 표시할 레이블 추가
        self.discharge_summary_label = ttk.Label(self.discharge_tab, text="TOTAL- 0개")
        self.discharge_summary_label.pack(pady=5)
    ...
    def setup_load_tab(self):
        ...
        # 요약 정보를 표시할 레이블 추가
        self.load_summary_label = ttk.Label(self.load_tab, text="TOTAL- 0개")
        self.load_summary_label.pack(pady=5)
    ...
    def generate_xml(self):
        ...
        try:
            ...
            pilot.set('type', 'Docking/Berthing')
            pilot.set('number', '1')
            ...
            # Arrival Linesmen (Mooring)
            linesmen = SubElement(arrival, 'linesmen')
            linesmen.set('cancelled', 'false')
            linemen = SubElement(linesmen, 'linemen')
            linemen.set('type', 'NO')
            linemen.set('number', '1')
            linemen.set('from', self.convert_datetime(self.arr_mooring_from_entry.get()))
            linemen.set('to', self.convert_datetime(self.arr_mooring_to_entry.get()))
            linemen.set('index', '1')
            ...
            self.add_timeline_field(timeline, 'DEPLOSTTIME', 'S', self.dep_reaforanc_var.get())
            
            # Departure Linesmen (Unmooring)
            linesmen = SubElement(departure, 'linesmen')
            linesmen.set('cancelled', 'false')
            linemen = SubElement(linesmen, 'linemen')
            linemen.set('type', 'NO')
            linemen.set('number', '1')
            linemen.set('from', self.convert_datetime(self.dep_mooring_from_entry.get()))
            linemen.set('to', self.convert_datetime(self.dep_mooring_to_entry.get()))
            linemen.set('index', '1')
    ...
    def generate_xml_partner(self):
        ...
        try:
            ...
            # # Draft 섹션
            # draft = SubElement(arrival, 'draft')
            # self.add_draft_item(draft, 'AFT', '')
            # self.add_draft_item(draft, 'FWD', '')
            ...
            linesmen.set('cancelled', 'false')
            linemen = SubElement(linesmen, 'linemen')
            linemen.set('type', 'Unmooring')
            linemen.set('number', '1')
            linemen.set('from', self.convert_datetime(self.dep_mooring_from_entry.get()))
            linemen.set('to', self.convert_datetime(self.dep_mooring_to_entry.get()))
            linemen.set('comment', 'Unmooring operation')
            linemen.set('index', '1')
    ...
    def handle_paste(self, tab_type):
        ...
        try:
            ...
            for fe in ['FULL', 'EMPTY']:
                for size in ['20', '40', '45']:
                    ...
                    if count > 0:
                        ...
                        # Operator별 컨테이너 수 업데이트
                        if new_operator not in operator_summary:
                            operator_summary[new_operator] = 0
                        operator_summary[new_operator] += count
            ...
            # 붙여넣기 횟수 증가 및 Operator별 컨테이너 수 기록
            if tab_type == 'discharge':
                self.discharge_paste_count += 1
                for operator, count in operator_summary.items():
                    if operator not in self.discharge_operator_counts:
                        self.discharge_operator_counts[operator] = []
                    self.discharge_operator_counts[operator].append(count)
                
                # 요약 정보 업데이트
                total_containers = sum(sum(counts) for counts in self.discharge_operator_counts.values())
                summary_text = f"TOTAL- {total_containers}개 ["
                operator_texts = []
                for operator, counts in self.discharge_operator_counts.items():
                    total = sum(counts)
                    operator_texts.append(f"{operator}- {total}개")
                summary_text += ", ".join(operator_texts) + "]"
                self.discharge_summary_label.config(text=summary_text)
            else:
                self.load_paste_count += 1
                for operator, count in operator_summary.items():
                    if operator not in self.load_operator_counts:
                        self.load_operator_counts[operator] = []
                    self.load_operator_counts[operator].append(count)
                
                # 요약 정보 업데이트
                total_containers = sum(sum(counts) for counts in self.load_operator_counts.values())
                summary_text = f"TOTAL- {total_containers}개 ["
                operator_texts = []
                for operator, counts in self.load_operator_counts.items():
                    total = sum(counts)
                    operator_texts.append(f"{operator}- {total}개")
                summary_text += ", ".join(operator_texts) + "]"
                self.load_summary_label.config(text=summary_text)
    ...
    def setup_terminal_efficiency_tab(self):
        ...
        efficiency_items = [
            ...
            ('CI_Sub_Optimal', '[Reason] Reason for being sub-optimal'),
            ('Starting_Operations', '[Reason] Were changes made to the load list less that  12 hours before starting operations?'),
            ...
            ('Reason_for_Live_Connections', '[Reason] Reason for Live Connections'),
            ('Affect_by_Cut_And_Run', 'Number of containers affected by Cut and Run'),
            ('Reason_For_Cut_and_Run', '[Reason] Reason For Cut and Run'),
    ...
    def save_to_json(self):
        try:
            ...
            # Get vessel, voyage, and port information
            vessel = self.vessel_entry.get()
            voyage = self.voyage_entry.get()
            port = self.portun_entry.get()
            
            # Create filename
            filename = f"{vessel}_{voyage}_{port}_{current_time}.json"
            ...
            data['header'] = {
                ...
                'voyage': voyage,
                'portun': port,
                'master': self.master_entry.get() if hasattr(self, 'master_entry') else '',
                'berth': self.berth_entry.get() if hasattr(self, 'berth_entry') else '',
                'general_remark': self.general_remark_text.get("1.0", "end-1c") if hasattr(self, 'general_remark_text') else ''
            }
            
            # Arrival tab data
            data['arrival'] = {
                'docatter': self.docatter_entry.get() if hasattr(self, 'docatter_entry') else '',
                'tugs': []
            }
            if hasattr(self, 'tug_frames_arrival'):
                for frame in self.tug_frames_arrival:
                    tug_data = {
                        'name': frame.tug_name_entry.get(),
                        'start': frame.tug_start_entry.get(),
                        'finish': frame.tug_finish_entry.get()
                    }
                    data['arrival']['tugs'].append(tug_data)
            
            # Operations tab data
            data['operations'] = {
                'gang_works': []
            }
            if hasattr(self, 'gang_work_frames'):
                for frame in self.gang_work_frames:
                    gang_data = {
                        'start': frame.gang_start_entry.get(),
                        'finish': frame.gang_finish_entry.get(),
                        'gangway_down': frame.gangway_down_entry.get(),
                        'lashing_completed': frame.lashing_completed_entry.get()
                    }
                    data['operations']['gang_works'].append(gang_data)
            
            # Departure tab data
            data['departure'] = {
                'docatter': self.docatter_entry_departure.get() if hasattr(self, 'docatter_entry_departure') else '',
                'tugs': []
            }
            if hasattr(self, 'tug_frames_departure'):
                for frame in self.tug_frames_departure:
                    tug_data = {
                        'name': frame.tug_name_entry.get(),
                        'start': frame.tug_start_entry.get(),
                        'finish': frame.tug_finish_entry.get()
                    }
                    data['departure']['tugs'].append(tug_data)
            
            # Discharge tab data
            data['discharge'] = {
                'containers': []
            }
            if hasattr(self, 'discharge_lines'):
                for line in self.discharge_lines:
                    container_data = {
                        'type': line['type'].get(),
                        'operator': line['operator'].get(),
                        'size': line['size'].get(),
                        'fe': line['fe'].get(),
                        'number': line['number'].get()
                    }
                    data['discharge']['containers'].append(container_data)
            
            # Load tab data
            data['load'] = {
                'containers': []
            }
            if hasattr(self, 'load_lines'):
                for line in self.load_lines:
                    container_data = {
                        'type': line['type'].get(),
                        'operator': line['operator'].get(),
                    ...
                    data['load']['containers'].append(container_data)
            
            # Shifting tab data
            data['shifting'] = {
                'lid_moves': []
            }
            if hasattr(self, 'shifting_lines'):
                for line in self.shifting_lines:
                    move_data = {
                        'account': line['account'].get(),
                        'type': line['type'].get(),
                        'size': line['size'].get(),
                        'reason': line['reason'].get(),
                        'value': line['value'].get(),
                        'fe': line['fe'].get(),
                        'oog': line['oog'].get(),
                        'reefer': line['reefer'].get(),
                        'imo': line['imo'].get(),
                        'notformscaccount': line['notformscaccount'].get()
                    }
                    data['shifting']['lid_moves'].append(move_data)
            
            # Terminal Efficiency tab data
            data['terminal_efficiency'] = {
                'starting_operations': self.starting_operations_entry.get() if hasattr(self, 'starting_operations_entry') else '',
                'total_containers': self.total_containers_entry.get() if hasattr(self, 'total_containers_entry') else '',
                'total_moves': self.total_moves_entry.get() if hasattr(self, 'total_moves_entry') else '',
                'total_time': self.total_time_entry.get() if hasattr(self, 'total_time_entry') else '',
                'total_containers_per_hour': self.total_containers_per_hour_entry.get() if hasattr(self, 'total_containers_per_hour_entry') else '',
                'total_moves_per_hour': self.total_moves_per_hour_entry.get() if hasattr(self, 'total_moves_per_hour_entry') else ''
            }
            ...
            messagebox.showinfo("Success", f"데이터가 성공적으로 저장되었습니다.\n저장 위치: {filename}")
            
        except Exception as e:
            messagebox.showerror("Error", f"데이터 저장 중 오류 발생: {str(e)}")
    ...
    def load_from_json(self):
        try:
            ...
            if 'header' in data:
                header = data['header']
                if hasattr(self, 'vessel_entry'):
                    self.vessel_entry.delete(0, tk.END)
                    self.vessel_entry.insert(0, header.get('vessel', ''))
                if hasattr(self, 'voyage_entry'):
                    self.voyage_entry.delete(0, tk.END)
                    self.voyage_entry.insert(0, header.get('voyage', ''))
                if hasattr(self, 'portun_entry'):
                    self.portun_entry.delete(0, tk.END)
                    self.portun_entry.insert(0, header.get('portun', ''))
                if hasattr(self, 'master_entry'):
                    self.master_entry.delete(0, tk.END)
                    self.master_entry.insert(0, header.get('master', ''))
                if hasattr(self, 'berth_entry'):
                    self.berth_entry.delete(0, tk.END)
                    self.berth_entry.insert(0, header.get('berth', ''))
                if hasattr(self, 'general_remark_text'):
                    self.general_remark_text.delete("1.0", tk.END)
                    self.general_remark_text.insert("1.0", header.get('general_remark', ''))
            
            # Arrival tab data
            if 'arrival' in data and hasattr(self, 'docatter_entry'):
                arrival = data['arrival']
                self.docatter_entry.delete(0, tk.END)
                self.docatter_entry.insert(0, arrival.get('docatter', ''))
                
                # Clear existing tug frames
                if hasattr(self, 'tug_frames_arrival'):
                    for frame in self.tug_frames_arrival:
                        self.delete_tug_frame(frame, 'arrival')
                    
                    # Add new tug frames with data
                    for tug_data in arrival.get('tugs', []):
                        frame = self.add_tug_frame(self.tug_frame_arrival, 'arrival')
                        frame.tug_name_entry.insert(0, tug_data.get('name', ''))
                        frame.tug_start_entry.insert(0, tug_data.get('start', ''))
                        frame.tug_finish_entry.insert(0, tug_data.get('finish', ''))
            
            # Operations tab data
            if 'operations' in data and hasattr(self, 'gang_work_frames'):
                operations = data['operations']
                
                # Clear existing gang work frames
                for frame in self.gang_work_frames:
                    self.delete_gang_work_line(frame)
                
                # Add new gang work frames with data
                for gang_data in operations.get('gang_works', []):
                    frame = self.add_gang_work_line()
                    frame.gang_start_entry.insert(0, gang_data.get('start', ''))
                    frame.gang_finish_entry.insert(0, gang_data.get('finish', ''))
                    frame.gangway_down_entry.insert(0, gang_data.get('gangway_down', ''))
                    frame.lashing_completed_entry.insert(0, gang_data.get('lashing_completed', ''))
            
            # Departure tab data
            if 'departure' in data and hasattr(self, 'docatter_entry_departure'):
                departure = data['departure']
                self.docatter_entry_departure.delete(0, tk.END)
                self.docatter_entry_departure.insert(0, departure.get('docatter', ''))
                
                # Clear existing tug frames
                if hasattr(self, 'tug_frames_departure'):
                    for frame in self.tug_frames_departure:
                        self.delete_tug_frame(frame, 'departure')
                    
                    # Add new tug frames with data
                    for tug_data in departure.get('tugs', []):
                        frame = self.add_tug_frame(self.tug_frame_departure, 'departure')
                        frame.tug_name_entry.insert(0, tug_data.get('name', ''))
                        frame.tug_start_entry.insert(0, tug_data.get('start', ''))
                        frame.tug_finish_entry.insert(0, tug_data.get('finish', ''))
            
            # Discharge tab data
            if 'discharge' in data and hasattr(self, 'container_frames_discharge'):
                discharge = data['discharge']
                
                # Clear existing container frames
                for frame in self.container_frames_discharge:
                    self.delete_container_line(frame, 'discharge')
                
                # Add new container frames with data
                for container_data in discharge.get('containers', []):
                    frame = self.add_container_line_with_data('discharge', container_data)
            
            # Load tab data
            if 'load' in data and hasattr(self, 'container_frames_load'):
                load = data['load']
                
                # Clear existing container frames
                for frame in self.container_frames_load:
                    self.delete_container_line(frame, 'load')
                
                # Add new container frames with data
                for container_data in load.get('containers', []):
                    frame = self.add_container_line_with_data('load', container_data)
            
            # Shifting tab data
            if 'shifting' in data and hasattr(self, 'shifting_frames'):
                shifting = data['shifting']
                
                # Clear existing shifting frames
                for frame in self.shifting_frames:
                    self.delete_shifting_line(frame)
                
                # Add new shifting frames with data
                for move_data in shifting.get('lid_moves', []):
                    frame = self.add_shifting_line()
                    frame.container_no_entry.insert(0, move_data.get('container_no', ''))
                    frame.from_entry.insert(0, move_data.get('from', ''))
                    frame.to_entry.insert(0, move_data.get('to', ''))
                    frame.time_entry.insert(0, move_data.get('time', ''))
            
            # Terminal Efficiency tab data
            if 'terminal_efficiency' in data:
                te = data['terminal_efficiency']
                if hasattr(self, 'starting_operations_entry'):
                    self.starting_operations_entry.delete(0, tk.END)
                    self.starting_operations_entry.insert(0, te.get('starting_operations', ''))
                if hasattr(self, 'total_containers_entry'):
                    self.total_containers_entry.delete(0, tk.END)
                    self.total_containers_entry.insert(0, te.get('total_containers', ''))
                if hasattr(self, 'total_moves_entry'):
                    self.total_moves_entry.delete(0, tk.END)
                    self.total_moves_entry.insert(0, te.get('total_moves', ''))
                if hasattr(self, 'total_time_entry'):
                    self.total_time_entry.delete(0, tk.END)
                    self.total_time_entry.insert(0, te.get('total_time', ''))
                if hasattr(self, 'total_containers_per_hour_entry'):
                    self.total_containers_per_hour_entry.delete(0, tk.END)
                    self.total_containers_per_hour_entry.insert(0, te.get('total_containers_per_hour', ''))
                if hasattr(self, 'total_moves_per_hour_entry'):
                    self.total_moves_per_hour_entry.delete(0, tk.END)
                    self.total_moves_per_hour_entry.insert(0, te.get('total_moves_per_hour', ''))
            
            messagebox.showinfo("Success", "데이터가 성공적으로 로드되었습니다.")
            
        except Exception as e:
            messagebox.showerror("Error", f"데이터 로드 중 오류 발생: {str(e)}")
