...
def parse_excel_data(data):
    ...
    try:
        ...
        if len(lines) > 0:
            ...
            results['DIMP']['FULL']['20'] = int(cells_dimp[2]) if len(cells_dimp) > 1 and cells_dimp[2].strip() and cells_dimp[2].strip().isdigit() else 0
            results['DIMP']['FULL']['40'] = (int(cells_dimp[3]) if len(cells_dimp) > 2 and cells_dimp[3].strip() and cells_dimp[3].strip().isdigit() else 0) + \
                                          (int(cells_dimp[4]) if len(cells_dimp) > 3 and cells_dimp[4].strip() and cells_dimp[4].strip().isdigit() else 0)
            results['DIMP']['FULL']['45'] = int(cells_dimp[5]) if len(cells_dimp) > 4 and cells_dimp[5].strip() and cells_dimp[5].strip().isdigit() else 0
            results['DIMP']['EMPTY']['20'] = int(cells_dimp[6]) if len(cells_dimp) > 5 and cells_dimp[6].strip() and cells_dimp[6].strip().isdigit() else 0
            results['DIMP']['EMPTY']['40'] = (int(cells_dimp[7]) if len(cells_dimp) > 6 and cells_dimp[7].strip() and cells_dimp[7].strip().isdigit() else 0) + \
                                           (int(cells_dimp[8]) if len(cells_dimp) > 7 and cells_dimp[8].strip() and cells_dimp[8].strip().isdigit() else 0)
            results['DIMP']['EMPTY']['45'] = int(cells_dimp[9]) if len(cells_dimp) > 8 and cells_dimp[9].strip() and cells_dimp[9].strip().isdigit() else 0
        ...
        if len(lines) > 1:
            ...
            # FULL
            results['TRAN']['FULL']['20'] = (int(cells_tran_row1[1]) if len(cells_tran_row1) > 1 and cells_tran_row1[1].strip() and cells_tran_row1[1].strip().isdigit() else 0) + \
                                          (int(cells_tran_row2[1]) if len(cells_tran_row2) > 1 and cells_tran_row2[1].strip() and cells_tran_row2[1].strip().isdigit() else 0)
            results['TRAN']['FULL']['40'] = (int(cells_tran_row1[2]) if len(cells_tran_row1) > 2 and cells_tran_row1[2].strip() and cells_tran_row1[2].strip().isdigit() else 0) + \
                                          (int(cells_tran_row1[3]) if len(cells_tran_row1) > 3 and cells_tran_row1[3].strip() and cells_tran_row1[3].strip().isdigit() else 0) + \
                                          (int(cells_tran_row2[2]) if len(cells_tran_row2) > 2 and cells_tran_row2[2].strip() and cells_tran_row2[2].strip().isdigit() else 0) + \
                                          (int(cells_tran_row2[3]) if len(cells_tran_row2) > 3 and cells_tran_row2[3].strip() and cells_tran_row2[3].strip().isdigit() else 0)
            results['TRAN']['FULL']['45'] = (int(cells_tran_row1[4]) if len(cells_tran_row1) > 4 and cells_tran_row1[4].strip() and cells_tran_row1[4].strip().isdigit() else 0) + \
                                          (int(cells_tran_row2[4]) if len(cells_tran_row2) > 4 and cells_tran_row2[4].strip() and cells_tran_row2[4].strip().isdigit() else 0)

            # EMPTY
            results['TRAN']['EMPTY']['20'] = (int(cells_tran_row1[5]) if len(cells_tran_row1) > 5 and cells_tran_row1[5].strip() and cells_tran_row1[5].strip().isdigit() else 0) + \
                                           (int(cells_tran_row2[5]) if len(cells_tran_row2) > 5 and cells_tran_row2[5].strip() and cells_tran_row2[5].strip().isdigit() else 0)
            results['TRAN']['EMPTY']['40'] = (int(cells_tran_row1[6]) if len(cells_tran_row1) > 6 and cells_tran_row1[6].strip() and cells_tran_row1[6].strip().isdigit() else 0) + \
                                           (int(cells_tran_row1[7]) if len(cells_tran_row1) > 7 and cells_tran_row1[7].strip() and cells_tran_row1[7].strip().isdigit() else 0) + \
                                           (int(cells_tran_row2[6]) if len(cells_tran_row2) > 6 and cells_tran_row2[6].strip() and cells_tran_row2[6].strip().isdigit() else 0) + \
                                           (int(cells_tran_row2[7]) if len(cells_tran_row2) > 7 and cells_tran_row2[7].strip() and cells_tran_row2[7].strip().isdigit() else 0)
            results['TRAN']['EMPTY']['45'] = (int(cells_tran_row1[8]) if len(cells_tran_row1) > 8 and cells_tran_row1[8].strip() and cells_tran_row1[8].strip().isdigit() else 0) + \
                                           (int(cells_tran_row2[8]) if len(cells_tran_row2) > 8 and cells_tran_row2[8].strip() and cells_tran_row2[8].strip().isdigit() else 0)
...
class VCIGeneratorGUI:
    ...
    def setup_operations_tab(self):
        ...
        gang_work_frame.pack(pady=5, fill='x', padx=70)
    ...
    def setup_discharge_tab(self):
        ...
        # 붙여넣기 버튼 추가
        paste_button = ttk.Button(self.discharge_tab, text="Paste Container Data",
                                command=lambda: self.handle_paste('discharge'))
        paste_button.pack(pady=5)
        
        # 빈 영역 클릭 시 붙여넣기를 위한 레이블 추가
        empty_area = ttk.Label(self.discharge_tab, text="Click here to paste data")
        empty_area.pack(pady=10, expand=True, fill='both')
    ...
    def setup_load_tab(self):
        ...
        # 붙여넣기 버튼 추가
        paste_button = ttk.Button(self.load_tab, text="Paste Container Data",
                                command=lambda: self.handle_paste('load'))
        paste_button.pack(pady=5)
        
        # 빈 영역 클릭 시 붙여넣기를 위한 레이블 추가
        empty_area = ttk.Label(self.load_tab, text="Click here to paste data")
        empty_area.pack(pady=10, expand=True, fill='both')
    ...
    def generate_xml(self):
        try:
            ...
            for line in self.gang_work_lines:
                ...
                # grosshours는 선택사항이므로 생략
                
                # workingperiodpayrate는 필수값 (기본값 "Normal")
                ...
                for size in ['20', '40', '45']:
                    ...
                    if count > 0:
                        container_data.append({
                            'type': 'DIMP',
                            'operator': clipboard_data.strip().split('\n')[0].split('\t')[0],
